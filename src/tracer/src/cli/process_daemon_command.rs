...
pub async fn process_daemon_command(command: Command, platform: PlatformInfo, config: Config) {
    let api_client = DaemonClient::new(format!("http://{}", config.server));
    match command {
        Command::Init(args) => handlers::init(*args, platform, config, api_client)
            .await
            .unwrap(),
        Command::Test(args) => handlers::test(*args, platform, config, api_client)
            .await
            .unwrap(),
