...
pub async fn handle_existing_daemon(args: &TracerCliInitArgs, api_client: &DaemonClient) -> anyhow::Result<()> {
    let pid_info = get_pid()
        .map(|pid| format!(" (PID {})", pid))
        .unwrap_or_default();

    if args.force {
        warning_message!("Daemon already running{}. Terminating due to --force flag...", pid_info);
